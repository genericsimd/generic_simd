...
#define TRANS_AGENT_SEND_BUF_MAX_LEN        (10240)
#define TRANS_AGENT_REV_BUF_MAX_LEN          (10240)
...
/*Revice Module*/
#define TRANS_AGENT_SPECTRUM_MGMT      ("53")    /* spectrum manager / configuration service*/
#define TRANS_AGENT_HEARTBEAT_MGMT      ("51")    /* Heartbeat manager Monitor service*/
...
enum trans_agent_bs_metric_id_enum
{
    ...
    TRANS_AGENT_BS_METRIC_ID_CRC = 0x0109,                /*CRCErrorCount*/   
    TRANS_AGENT_BS_METRIC_ID_PDU = 0x010a,                /*PDUTotalCount*/    
...
enum trans_agent_ms_metric_id_enum
{
    ...
    TRANS_AGENT_MS_METRIC_ID_CRC = 0x0305,     /*CRCErrorCount*/    
    TRANS_AGENT_MS_METRIC_ID_PDU = 0x0306,       /*PDUTotalCount*/    
...
struct trans_agent_metric_info
{
    ...
    int32_t w_dest_id;
    //int32_t w_module_id;
    u_int8_t a_module_id[TRANS_AGENT_MODULE_ID_SIZE + 1];
...
struct trans_agent_idupdate_info
{
    u_int8_t    a_device_id[TRANS_AGENT_MODULE_ID_SIZE + 1];
};

#define SIZEOF_TRANS_AGENT_IDUPDATE_INFO     sizeof(struct trans_agent_idupdate_info)
...
struct trans_agent_confupdate_info
{
    float f_freq;

};

#define SIZEOF_TRANS_AGENT_CONFUPDATE_INFO     sizeof(struct trans_agent_confupdate_info)
...
extern u_int32_t trans_agent_parse_msg(void *p_info, 
                           size_t len,
                           void * p_rev_buf);

extern u_int32_t trans_agent_rev_msg(u_int8_t **pp_rev_msg, int32_t w_agent_socket, int32_t *p_len);
...
extern u_int32_t trans_agent_send_state_msg();

extern u_int32_t trans_agent_send_conn_msg();

extern u_int32_t trans_agent_hb_timer(void);
...
extern void *g_trans_agent_hb_timer;
