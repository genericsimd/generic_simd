...
const char wk_hex_1024fft1[114][WK_HEX_LENGTH+10] =
{
                               ...
                               float *output_fi)
{
    ...
    float preamble_fr[OFDMA_SYMBOL_SIZE], preamble_frr[OFDMA_SYMBOL_SIZE];
    u_int32_t preamble_temp[OFDMA_SYMBOL_SIZE];
    ...
    {
	memcpy(bitsel, active_band, 15*sizeof(char));
    }

    p_wk_hex = &wk_hex_1024fft1[para->preamble_index][0];
    ...
    preamble_unshift[para->ofdma_nfft/2] = 0;
 ...
 /* store the index of used subcarrier */
    usedsc =  para->datasub_persubsym + para->pilot_persubsym;
    for (i=0; i<count; i++)
    {
        for (j=0; j<usedsc; j++)
        {
            preamble_temp[i*usedsc+j] = sc_allocation[usedbit[i]*usedsc+j];
        }
    }
   
    memset(preamble_fr,0,para->ofdma_nfft*sizeof(float));
    offset = para->preamble_guard_band;
    for(i=0; i<WK_HEX_LENGTH*4*3; i++)
    {
        preamble_fr[i+offset] = preamble_unshift[i];
    }

    memset(preamble_frr,0,para->ofdma_nfft*sizeof(float));
    for (i=0; i<count*usedsc; i++)
    {
        preamble_frr[preamble_temp[i]] = preamble_fr[preamble_temp[i]];
    }    
#if 0
    FILE *fp6;
    fp6 = fopen("./data_dst/1_preamble_temp.out","w+t");
    dl_dump_ffloat(fp6, preamble_frr, 1024);
    fclose(fp6);
#endif



 //   if ( para->segment_index == 0 ) preamble_unshift[para->ofdma_dc] = 0;

    DO_DUMP(DUMP_PHY_TX_PREAMBLE_TEMP_ID, 0, preamble_unshift, 1024);
    ...
    preamble_boost = sqrt((float)756/(float)(count*18));
  //  printf("--------------preamble_boost = %f-------------------------\n", preamble_boost);
    for (sample_count=0; sample_count<para->ofdma_dc; sample_count++) {
        output_fr[sample_count] = preamble_boost * preamble_frr[sample_count+para->ofdma_dc];
        output_fr[sample_count+para->ofdma_dc] = preamble_boost * preamble_frr[sample_count];
    }
...
#if 0
    FILE *fp5_i;
    fp5_i = fopen("./data_dst/1_preamble_fre_i.out","w+t");
   
    dl_dump_ffloat(fp5_i, output_fr, 1024);
    fclose(fp5_i);
#endif
                              ...
                              float *output_ti)
{
    ...
    err_code = phy_dl_preamble_fgen_multi(para,active_band, sc_allocation, preamble_fr, preamble_fi);
    if(err_code) {
