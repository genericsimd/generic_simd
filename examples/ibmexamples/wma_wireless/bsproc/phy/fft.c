...
/* For 1024-FFT only!!! */
int fft_init_p(unsigned int N, float ***XX, float ***x, float ***X)
{
    int i,j;
    float *W;
...
void fft_p(unsigned int N, float (*x_r), float (*x_i), float (*X_R), float (*X_I),float **XX, float **x, float **X)
{
    unsigned int i, j;
...
/* IFFT */
void ifft_p(unsigned int N, float (*x_r), float (*x_i), float (*X_R), float (*X_I), float **XX, float **x, float **X)
{
    unsigned int i, j;
    vector float xr, xi, x1, x2;
    vector unsigned char vmask1 = {0x00, 0x01, 0x02, 0x03, 0x08, 0x09, 0x0A, 0x0B, 0x10, 0x11, 0x12, 0x13, 0x18, 0x19, 0x1A, 0x1B};
...
#if defined VSXOPT
    fft_init_p( N, XX, x, X);
    fft_init_p1( N, XX, x, X);
    fft_init_p2( N, XX, x, X);
    fft_init_p3( N, XX, x, X);
#endif
    return size;
}
