...
/* For 1024-FFT only!!! */
int fft_init_p2(unsigned int N, float ***XX, float ***x, float ***X)
{
    int i,j;
    float *W;
    posix_memalign((void **)&aux12, 16, sizeof(float)*(NFFT)*2);
    posix_memalign((void **)&aux22,  16, sizeof(float)*(NFFT)*2);
    posix_memalign((void **)&W, 16, sizeof(float)*(NFFT)*2);
    posix_memalign((void **)&WTF2,16, sizeof(float)*(NFFT)*2);
    posix_memalign((void **)&WFT2,16, sizeof(float)*(NFFT)*2);
    ...
    for (j=0; j<16; j++) {
      for (i=0; i<16; i++) {
        WTF2[128*j+8*i  ] = W[4*(i+1)*j      ];
        WTF2[128*j+8*i+1] = W[4*(i+1)*j      ];
        WTF2[128*j+8*i+2] = W[4*(i+1)*j+2*(i+1)  ];
        WTF2[128*j+8*i+3] = W[4*(i+1)*j+2*(i+1)  ];
        WTF2[128*j+8*i+4] =-W[4*(i+1)*j    +1];
        WTF2[128*j+8*i+5] = W[4*(i+1)*j    +1];
        WTF2[128*j+8*i+6] =-W[4*(i+1)*j+2*(i+1)+1];
        WTF2[128*j+8*i+7] = W[4*(i+1)*j+2*(i+1)+1];
      }
    }
    ...
    for (j=0; j<16; j++) {
      for (i=0; i<16; i++) {
        WFT2[128*j+8*i  ] = 1.0/(NFFT) *W[4*i*j];
        WFT2[128*j+8*i+1] = 1.0/(NFFT) *W[4*i*j];
        WFT2[128*j+8*i+2] = 1.0/(NFFT) *W[4*i*j+i];
        WFT2[128*j+8*i+3] = 1.0/(NFFT) *W[4*i*j+i];
        WFT2[128*j+8*i+4] =-1.0/(NFFT) *W[4*i*j  +1];
        WFT2[128*j+8*i+5] = 1.0/(NFFT) *W[4*i*j  +1];
        WFT2[128*j+8*i+6] =-1.0/(NFFT) *W[4*i*j+i+1];
        WFT2[128*j+8*i+7] = 1.0/(NFFT) *W[4*i*j+i+1];
      }
    }
    free(W);
    return 0;
...
void fft_p2(unsigned int N, float (*x_r), float (*x_i), float (*X_R), float (*X_I),float **XX, float **x, float **X)
{
    unsigned int i, j;
...
for (j = 0; j < 100; j++) {
    ...
    for (i=0;i<N/4;i++) {
        ...
        x1 = vec_mergeh(xr, xi);
        x2 = vec_mergel(xr, xi);
        vec_st(x1, 0, &aux12[i*8]);
        vec_st(x2,16, &aux12[i*8]);
    }


    /* Calculate FFT by a recursion. */
    fft_32_s1(1, 1.0, aux12, aux22, WTF2);
    fft_32_s2(1, aux22, aux12);

    for (i=0;i<N/4;i++) {
        x1 = vec_ld( 0, &aux12[i*8]);
        x2 = vec_ld(16, &aux12[i*8]);
...
#if 1
/* IFFT */
void ifft_p2(unsigned int N, float (*x_r), float (*x_i), float (*X_R), float (*X_I), float **XX, float **x, float **X)
{
    unsigned int i, j;
    vector float xr, xi, x1, x2;
    vector unsigned char vmask1 = {0x00, 0x01, 0x02, 0x03, 0x08, 0x09, 0x0A, 0x0B, 0x10, 0x11, 0x12, 0x13, 0x18, 0x19, 0x1A, 0x1B};
...
for (j = 0; j < 100; j++) {
    ...
    /* Calculate FFT by a recursion. */
    fft_32_s1(-1, 1.0/(NFFT), aux12, aux22, WFT2);
    fft_32_s2(-1, aux22, aux12);
