                                     ...
                                     u_int32_t *data_allocation)
{
  ...
  //  int8_t fch_logsub_selbit[21]; //FCH logic bit selection
    ...
    u_int32_t num_datasc_slotsym = 0;
   ...
   // fch_startphysub = 14*para->segment_index + 4*(para->id_cell%3);
     fch_startphysub = 5*para->segment_index;
   /*
...
*/	
	...
	bitsel_index[5+i] = para->fch_bitmap[2];
	bitsel_index[10+i] = para->fch_bitmap[4];
    }
    bitsel_index[4] = para->fch_bitmap[1];
    bitsel_index[9] = para->fch_bitmap[3];
    bitsel_index[14] = para->fch_bitmap[5];
	
   /* getting the real active logic channel */
    for (i=0; i< subband_num; i++)
    {
        active_band[i] = dts_selbit[i] ^ bitsel_index[i]; //1-can use, 0--can not be used
       // printf("---------------------active_band = %d--------------------------------\n", active_band[i]);
    }
    ...
    for (i=1; i< (para->num_phyband+1); i++)
    {
        subch_phyindex[i-1] = i * active_band[i-1];
      //  printf("----------------subch_phyindex= %d-----------------\n", subch_phyindex[i-1]);
    }
    ...
    for (i=0; i<fch_startphysub; i++)
    {
      ...
      //  printf("---------------------subch_logindex = %d--------------------------------\n",
    //            subch_logindex[i+para->num_subch-fch_startphysub]);
    }
 ...
 // printf("-------------------------------num_unused_subch = %d----------------------\n", num_unused_subch);
#if 0
    FILE *fp7;
    fp7 = fopen("./data_dst/2_subch_lognon.out","w+t");
    dl_dump_uinteger(fp7, subch_lognon,15);
    fclose(fp7);
#endif
...
#ifdef _ZONE_BOOST_
    float zone_boost;
    zone_boost = sqrt( (para->ofdma_nused-1) / (num_used_subch*18));
#endif
...
//  memcpy(dlusedsc, para->dlusesc, 18*15*sizeof(u_int32_t));
    for (i=0; i< para->symbol_per_slot; i++)
    {
         memcpy(dlusedsc[i],para->dlusesc, 18*15*sizeof(u_int32_t));

        for (j=0; j<para->num_bins; j++)
        {
            pilot_per_slotsym [i*para->num_bins+j] = para->dlusesc[init_pilot_index[j]+3*i];
            dlusedsc[i][init_pilot_index[j]+3*i] = 0;
        }            	   
    }


 DO_DUMP(DUMP_PHY_DL_ZONEPERM_PILOT_TMP_ID, 0, pilot_per_slotsym, para->num_bins*3);
 ...
 /*select the pilot for used subchannels */
    ...
    num_datasc_slotsym = para->num_subch*para->datasub_persubsym;
    memset(data_per_slotsym, 0, num_datasc_slotsym*para->symbol_per_slot*sizeof(u_int32_t));
    for (i=0; i<para->symbol_per_slot; i++)
    {
        counter = 0;
        for (j=0; j<18*15; j++)
        {
            ...
            {
                counter++;
                data_per_slotsym[i*num_datasc_slotsym + counter-1] = dlusedsc[i][j];
            }                
        }
    }
    

    DO_DUMP(DUMP_PHY_DL_ZONEPERM_DATA_TMP_ID, 0, data_per_slotsym, para->num_data_subcar*3);
#if 0
    FILE *fp8;
    fp8 = fopen("./data_dst/2_data_per_slotsym.out","w+t");
    dl_dump_uinteger(fp8, data_per_slotsym,16*15*3);
    fclose(fp8);
#endif
...
#if 0
    FILE *fp10;
    fp10 = fopen("./data_dst/2_data_allocation_temp.out","w+t");
    dl_dump_uinteger(fp10, data_allocation_temp,16*15*3);
    fclose(fp10);
#endif
...
#if 0
    FILE *fp9;
    fp9 = fopen("./data_dst/2_data_allocation_reshape.out","w+t");
    dl_dump_uinteger(fp9, data_allocation_reshape,temp*3);
    fclose(fp9);
#endif
...
//	DO_DUMP(DUMP_PHY_DL_ZONEPERM_DATA_ZONE_ID, 0, data_allocation_reshape, used_datanum);
    ...
    for (i=0; i<42; i++)
    {
        ...
        //    printf("----------------permbase_1st= %d------------------\n", permbase_1st[i-amc_count]);
        }
        else
        {
            amc_count++;
...
#if 0
    FILE *fp11;
    fp11 = fopen("./data_dst/2_data_index_1st.out","w+t");
    dl_dump_uinteger(fp11, dataindex_1st,subchnum_1st*16);
    fclose(fp11);
#endif

    /* generate dataindex for other symbols (not the 1st slotsymbol) */
    amc_count = 0;
    memset(permbase_1st, 0, 42*sizeof(u_int32_t));
    for (i=0; i<42; i++)
    {
...
#if 0
    FILE *fp11;
    fp11 = fopen("./data_dst/2_data_index_others.out","w+t");
    dl_dump_uinteger(fp11, dataindex_others,num_used_subch*16);
    fclose(fp11);
#endif
...
#if 0 
    FILE *fp11;
    fp11 = fopen("./data_dst/2_data_amc_rotation.out","a+t");
    dl_dump_uinteger(fp11, data_amc_rotation, num_used_subch*48);
    fclose(fp11);
#endif
...
#if 0
    FILE *fp11;
    fp11 = fopen("./data_dst/2_data_allocation_reshape.out","a+t");
    dl_dump_uinteger(fp11, data_allocation_reshape, num_used_subch*48);
    fclose(fp11);
#endif
...
#if 0
    FILE *fp12;
    fp12 = fopen("./data_dst/2_data_allocation_internal.out","a+t");
    dl_dump_uinteger(fp12, data_allocation, num_used_subch*48);
    fclose(fp12);
#endif
